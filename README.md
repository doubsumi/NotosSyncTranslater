...
| **句级队列协议（核心）** | 每侧 = 一个 `docId` 文档队列；句子为带稳定 `sid` 的缓存单元。**每侧面板只显示它自己的语言**：待翻译句子留空、结果到达后逐句填入；编辑只把**被改句子的 sid+新文本**上送后端队列，后端只处理并返回该 sid，前端只更新该 sid 的文本区域（绝不把源语言当作占位写入另一侧，杜绝中英混排） |
...
- **单语言面板 + 逐句流式**：每一侧只显示它自己的语言（原文侧原文、译文侧译文）。待翻译的句子留空，随后端逐句返回原地填入；修改某句时仅该句区域清空并等待新译文（不显示源语言占位，避免中英混排）。
...
3. **单语言面板渲染**：目标面板 = 源布局 + 每句译文（未返回/失败的句子**留空**，绝不回填源语言占位——这是“英文侧被整侧刷成中文”的根因），结果逐句原位填入；CodeMirror 以最小差异事务只改该句区间。
