...
| **只请求修改部分（句子级）** | 长文先按换行分块（双栏布局 1:1），块内按句切分；改第 N 句只上送第 N 句，未改句子与段落从记忆拼回，**不整段重译** |
| 双侧滚动联动 | 滚动一侧输入框，另一侧按比例同步滚动 |
| 选区对应高亮 | 在一侧选中文本，另一侧自动选中并滚动到对应原文/译文（段级精确 + 段内比例映射） |
...
1. **句子级分块与翻译记忆**
   文档 → 换行块（布局 1:1，供滚动/选区联动）→ 句级切分（缓存单元）。缓存键为 `sha1(from·to·text)`，**O(1) 命中**；未命中句由线程池（≤3 并发）并行请求，批内自动去重、请求间按键去重（in-flight 合并）。总体复杂度 O(n)、单次上游请求负载恒定，因此**没有输入长度上限**；前端因此能做到“改哪句请求哪句”。
